...
- [ ] **Goal:** Get to a clean working state on a fresh branch without losing salvageable code.

**Tasks**
- [ ] On a new branch `v3-revival` (cut from `main`), triage the 18 uncommitted changes currently on the `Azure` branch — keep nothing automatically; review each modified file and decide.
- [ ] Delete `dist/` from version control entirely. Add `dist/` to `.gitignore` (Cloudflare Pages will build fresh on each deploy).
- [ ] Delete the obsolete `data/all_instances.json` (5MB) and `data/summary.json`. They will be regenerated by the new aggregator (Stage 6).
- [ ] Update [scripts/orchestrator.py](scripts/orchestrator.py) `PROVIDER_CONFIG` so only `aws`, `azure`, `gcp`, `oci` are `enabled: True`. Set every secondary provider (hetzner, ovh, digitalocean, linode, scaleway, vultr, contabo) to `enabled: False` with a comment "v3.1 — disabled for v1 launch".
- [ ] Move `data/providers/{hetzner,ovh,digitalocean,linode,scaleway,vultr,contabo}.json` to `data/providers/_archive/` so they don't accidentally ship.
- [ ] Add a `_redirects` and `_headers` file at repo root (placeholders — Stage 9 fills them in).
...
**Definition of Done**
- [ ] PR opened: `v3/stage-0-hygiene` → `v3-revival`
- [ ] CI green (existing lint/type-check)
